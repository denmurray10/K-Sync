...
def api_schedule_data(request):
    ...
    for s in schedules:
        ...
        resolved_host = s.host
        if _is_placeholder_scheduler_host(resolved_host):
            resolved_host = (
                assigned_host_by_playlist.get(s.playlist.id)
                or assigned_host_by_day.get(s.day)
        ...
        day_map[s.day].append({
            ...
            'name': _sanitize_playlist_name(s.playlist.name),
            ...
            'host': _display_host_name(resolved_host),
            'description': (
                playlist_preview_by_id.get(s.playlist.id)
                or s.description
